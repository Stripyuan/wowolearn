	...
	if ($.fn.uploadify) {
		$(":file[uploaderOption]", $p).each(function(){
			...
			var options = {
				...
				onUploadSuccess:myUploadSuccess
			};
...
function navTabAjaxDone(json){
	...
	if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok){
		...
		//Jasmine 修改增加Yii的验证
		var form_name = json['form-name'];
		var errors    = json['errors'];
		for(x in errors){
			var input_id = form_name.concat('-',x);
			$("#"+input_id).addClass('error');
			$("span[for="+input_id+"]").text(errors[x]);
			$("span[for="+input_id+"]").addClass('error');
			$("span[for="+input_id+"]").css('display','block');
		}
	}
}
...
function myUploadSuccess(file,data,response){
	var data = DWZ.jsonEval(data);
	$("#"+data.id+"").attr('value',data.name);
}
			...
			$this.click(function(event){
				...
				if (title) {
					alertMsg.confirm(title, {
						okCall: function(){
							ajaxTodo(url, $this.attr("callback"),{_csrf:_csrf});
						}
					});
				} else {
					ajaxTodo(url, $this.attr("callback"),{_csrf:_csrf});
						...
						$.ajax({
							...
							data: function(){
								if (postType == 'map'){
									...
									data[data.length] = {name: '_csrf', value: _csrf};
									return data;
								} else {
									var _data = {};
